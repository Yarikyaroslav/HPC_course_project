...
void simulateWithThreads(int num_threads, const std::string& initial_conditions_file) {
    omp_set_num_threads(num_threads);

    const double dt = 1.0E-04;
    const int Nsteps = 10000;

    // Load initial conditions from file
    std::vector<Body> bodies = loadInitialConditions(initial_conditions_file);
    ...
    std::cout << "Number of threads: " << num_threads << ", Time: " << elapsed_time << " seconds" << std::endl;

}

int main() {
    std::vector<int> thread_counts = {1, 2, 4, 6, 8};
    std::string initial_conditions_file = "initial_conditions.txt";

    for (int num_threads : thread_counts) {
        simulateWithThreads(num_threads, initial_conditions_file);
    }
